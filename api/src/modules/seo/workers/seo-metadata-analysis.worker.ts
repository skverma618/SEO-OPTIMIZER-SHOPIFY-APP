...
import {
  SeoMetadataSchema,
  type SeoMetadataAnalysis,
} from '../prompts/schemas';
import {
  SYSTEM_PROMPTS,
  createBrandAwareSystemPrompt,
...
export class SeoMetadataAnalysisWorker {
  ...
  async analyzeSeoMetadata(
    input: ProductSeoAnalysisInputDto,
    ...
    try {
      ...
      const baseSystemPrompt =
        SYSTEM_PROMPTS.SEO_EXPERT + '\n\n' + SYSTEM_PROMPTS.METADATA_SPECIALIST;
      const brandAwareSystemPrompt = createBrandAwareSystemPrompt(
        baseSystemPrompt,
        brandMapping,
      );
      ...
      const analysisResult: SeoMetadataAnalysis =
        await chain.invoke(formattedPrompt);

      // Transform suggestions to include proper IDs
      const suggestions = analysisResult.suggestions.map(
        (suggestion, index) => ({
          id: input.productId,
        ...
        fieldScores = fieldScores.map((fs) => {
          if (fs.score === 0 || !fs.description) {
            ...
            if (fieldSuggestions.length > 0) {
              ...
              const hasHighPriority = fieldSuggestions.some(
                (s) => s.priority === SuggestionPriority.HIGH,
              );
              const hasMediumPriority = fieldSuggestions.some(
                (s) => s.priority === SuggestionPriority.MEDIUM,
              );
            ...
            return {
              ...
              description:
                calculatedScore < 60
                  ? `${fs.field} needs significant improvement`
                  : calculatedScore < 80
                    ? `${fs.field} needs minor optimization`
      ...
      if (overallScore === 0) {
        overallScore = Math.round(
          fieldScores.reduce((sum, fs) => sum + fs.score, 0) /
            fieldScores.length,
        );
      }
  ...
  private mapSuggestionType(type: string): SuggestionType {
    switch (type.toLowerCase()) {
      ...
      case 'seo-title':
      case 'meta-title':
        return SuggestionType.META_TITLE;
  ...
  private getFallbackAnalysis(
    ...
    if (!input.productSeoTitle || input.productSeoTitle.length === 0) {
      suggestions.push({
        ...
        type: SuggestionType.META_TITLE,
        ...
        impact:
          'SEO titles are crucial for search rankings and click-through rates',
      });
      seoTitleScore = 25;
    } else if (
      input.productSeoTitle.length < 30 ||
      input.productSeoTitle.length > 60
    ) {
      suggestions.push({
        ...
        reason:
          'SEO title length should be between 30-60 characters for optimal display',
    ...
    if (
      !input.productSeoDescription ||
      input.productSeoDescription.length === 0
    ) {
      suggestions.push({
        ...
        suggested:
          'Discover premium quality products with excellent features. Shop now for the best deals and fast shipping. Perfect for your needs.',
        reason: 'Missing meta description reduces search engine visibility',
        impact:
          'Meta descriptions can improve click-through rates by up to 30%',
      });
      metaDescriptionScore = 30;
    } else if (
      input.productSeoDescription.length < 120 ||
      input.productSeoDescription.length > 160
    ) {
      suggestions.push({
        ...
        impact:
          'Proper meta description length improves search result appearance',
      });
    ...
    const fieldScores = [
      {
        ...
        description:
          seoTitleScore < 60
            ? 'SEO title needs optimization for length and content'
            : 'SEO title is well optimized',
      },
      {
        ...
        description:
          metaDescriptionScore < 60
            ? 'Meta description needs optimization for length and content'
    ...
    return {
      ...
      analysisType: 'seo-metadata',
      feedback:
        'Fallback analysis: Basic SEO metadata checks completed. Focus on optimizing title and description lengths.',
      fieldScores,
    };
  }
}
